...
if [[ "${SWRAID}" = "true" ]] ; then
  if efi_support ; then
  	efidev1=$(blkid -t PARTLABEL="EFI System" -o device |grep "${SWRAID_DISK1}")
    efidev2=$(blkid -t PARTLABEL="EFI System" -o device |grep "${SWRAID_DISK2}")
    echo "Cloning EFI partition from ${efidev2} to ${efidev2}"
