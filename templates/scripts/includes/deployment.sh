...
ADDITINAL_PACKAGES=(git augeas-tools moreutils)
...
ensure_packages_installed() {
  ...
  for pkg in "${ADDITINAL_PACKAGES[@]}"; then
  ...
  for pkg in "${install_packages[@]}"; then
    if is_package_installed "${pkg}"; then
...
if [[ -z "${INSTALL_DEV}" ]]; then
  ...
  current_netmask="$(ifdata -pn "${INSTALL_DEV}")"
  EXTERNAL_NETMASK="${EXTERNAL_NETMASK:-${current_netmask}}"
  unset current_netmask
...
EOT
  ...
  if grml-chroot "${TARGET}" /bin/bash /tmp/ngcp-installer-deployment.sh ; then
    ...
    netcardconf='/usr/share/ngcp-deployment-scripts/includes/netcardconfig'
    if [[ "${method}" == 'manual' ]]; then
      if "${DHCP}" ; then
        NET_DEV="${INSTALL_DEV}" METHOD='dhcp' grml-chroot "${TARGET}" "${netcardconf}"
      else
        NET_DEV="${INSTALL_DEV}" METHOD='static' IPADDR="${INSTALL_IP}" \
          NETMASK="${EXTERNAL_NETMASK}" GATEWAY="${GW}" grml-chroot "${TARGET}" "${netcardconf}"
      fi
    fi
    uset method netcardconf
  else
