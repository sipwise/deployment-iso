...
if checkBootParam 'ngcpinstallfromnode' ; then
  INSTALL_FROM_NODE=true
fi
...
INSTALL_FROM_NODE="${INSTALL_FROM_NODE}"
EOF
  fi
  ...
  if grml-chroot "${TARGET}" /bin/bash /tmp/ngcp-installer-deployment.sh ; then
    ...
    if [[ "${method}" == 'manual' ]]; then
      if "${DHCP}" ; then
        ...
        if "${PRO_EDITION}" && "${INSTALL_FROM_NODE}" ; then
...
if [[ "${SWRAID}" = "true" ]] ; then
  if efi_support ; then
    ...
    dd if="/dev/${SWRAID_DISK1}2" of="/dev/${SWRAID_DISK2}2" bs=10M
  fi
fi
