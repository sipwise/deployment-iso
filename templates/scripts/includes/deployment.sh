...
SIPWISE_APT_KEY_URL="\${SIPWISE_URL}/spce/sipwise.gpg"
...
install_sipwise_key() {
  ...
  for try in 1 2 3; do
    wget --retry-connrefused --no-verbose -O "${SIPWISE_APT_KEY_PATH}" "${SIPWISE_APT_KEY_URL}"
    sipwise_key_checksum=$(sha256sum "${SIPWISE_APT_KEY_PATH}" | awk '{print $1}')
...
if checkBootParam ngcpce ; then
  ...
  SIPWISE_APT_KEY_URL="\${SIPWISE_URL}/sppro/sipwise.gpg"
