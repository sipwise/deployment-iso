...
NETCARDCONFIG_USED="${NETCARDCONFIG_USED:-false}"
...
install_package_moreutils() {
  echo "Installing package moreutils (it is missed on GRML 'small')"
  ...
  # Can not used tmp workaround here because moreutils has some unmet
  # dependencies
...
create_interfaces_file() {
  local int_file="${1:?File for create_interfaces_file has to be defined}"
  cat >> "${int_file}" << EOF
auto lo
...
EOF
  ...
  if [[ -r /sys/class/net/bonding_masters ]]; then
    bond_devs="$(cat /sys/class/net/bonding_masters)"
  fi

  for net_dev in ${NETWORK_DEVICES}; do
    local parent_dev=''
    local slaves=''
    local method=''
    local ip=''
    ...
    if [[ -r /proc/net/vlan/config ]]; then
      if grep -q -E "^${net_dev} " /proc/net/vlan/config ; then
        parent_dev="$( awk -F ' *\\| *' '$1 == "${net_dev}" { print $3 }' /proc/net/vlan/config )"
      fi
    fi

    # bond interface ?
    for i in ${bond_devs}; do
      if [[ "${i}" == "${net_dev}" ]]; then
        slaves="$( cat "/sys/class/net/${net_dev}/bonding/slaves" )"
      fi
    done

    # DHCP basic ?
    local dhclient_pid
    dhclient_pid="$( pgrep dhclient || true )"
    if [[ -n "${dhclient_pid}" ]]; then
      if xargs -0 echo < "/proc/${dhclient_pid}/cmdline" | grep -E "${net_dev}$" ; then
        method='dhcp'
      fi
    else
      # last chance - dhcp can be specified in ip= boot parameter
      if [[ "${IP_ARR[device]}" == "${net_dev}" ]] && [[ "${IP_ARR[autoconf]}" == 'dhcp' ]]; then
        method='dhcp'
      else
        # get current parameters
        method='static'
        ip="$( ifdata -pa "${net_dev}" )"
        netmask="$( ifdata -pn "${net_dev}" )"
        # gw device ?
        gateway="$( ip route show dev "${net_dev}" | awk '/^default via/ {print $3}' )"

        # if there is no assigned ip - just skip this interface
        if [[ "${ip}" == 'NON-IP' ]]; then
          method='manual'
        fi
      fi
...
EOF
    ...
    if [[ -n "${parent_dev}" ]]; then
      echo "  vlan-raw-device ${parent_dev}" >> "${file}"
    fi
...
if ! "${NETCARDCONFIG_USED}" ; then
  install_package_moreutils
  echo "Generate ${TARGET}/etc/network/interfaces from current configuration"
  declare file
  file=$(mktemp)
  create_interfaces_file "${file}"
  cp "${file}" "${TARGET}/etc/network/interfaces"
  unset file
fi
