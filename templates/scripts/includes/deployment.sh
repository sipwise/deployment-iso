...
if "$PRO_EDITION" ; then
  ...
  echo "## Generated by Sipwise deployment script" > \
    "${TARGET_UDEV_PERSISTENT_NET_RULES:?}"
  for dev in ${NETWORK_DEVICES}; do
    [[ "${dev}" =~ ^lo ]] && continue

    pcid=$(udevadm info -a -p "/sys/class/net/${dev}" | sed -nr 's/^ +KERNELS=="([0-9]{4}:[0-9]{2}:[0-9]{2}.[0-9]+)".*$/\1/p')
    if [[ "${pcid}" =~ ^([0-9:.])+$ ]]; then
      echo "Adding device '${dev}' with PCID '${pcid}'"
      cat >> "${TARGET_UDEV_PERSISTENT_NET_RULES:?}" <<EOL
SUBSYSTEM=="net", ACTION=="add", DRIVERS=="?*", KERNELS=="${pcid}", NAME="${dev}"
EOL
    else
      echo "WARNING: could not find valid PCID for device '${dev}'"
    fi
  done
  unset pcid
fi
